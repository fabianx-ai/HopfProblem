...
theorem SphereBasepoint.exists_adjustment {Y : Type*} [TopologicalSpace Y] {y : Y}
  ...
  let side : C((unitInterval) × DiskCylinder.Sphere (E := V), Y) :=
    P.toContinuousMap.comp ContinuousMap.fst
  have h0 : ∀ s, side (0, s) = f (DiskCylinder.boundaryToDisk s) := by
    intro s
    have hs :=
      (DiskCube.boundary_iff L (DiskCylinder.boundaryToDisk s)).mpr
        (mem_sphere_zero_iff_norm.mp s.property)
  ...
  · intro z
    ...
    exact
      (hG 0 w).trans
        ((DiskCylinder.extend_bottom f side h0 (e.symm w)).trans
...
theorem Sphere.homotopic_id_of_topClass
  ...
  obtain ⟨v, hv, hgv⟩ :=
    SphereBasepoint.exists_adjustment g
