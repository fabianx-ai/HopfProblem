...
| `Lib/Topology/Homotopy/HandleRetraction.lean` | SH 5310–5859 (Handle retraction) | 68 | 720a5ae (listed under D1 lane execution, D2 dependency) |
...
1. DT tail (26 decls) depending on `MorseSurgeryData`/`Hemisphere.*` (F/G).
2. `MorseCells.built_of_compact_smooth_manifold` + `built_upper_sublevels`
...
- `exists_tubularNeighborhood_in_open_of_embedded_closedBall`:
  ...
  (the plan's `NativeEuclideanEmbedding.exists_tubularNeighborhood` spelling names a
...
The lane's namespaces (`ManifoldMorse.MorseSurgeryData`,
`ManifoldMorse.SurgeryWindows`, `Hemisphere.*`, `DiskDouble.*`,
`NoExotic.*`, ...) keep their names this lane, consistent with the D1/D2
precedent (HandleRetraction, Collar, RegularLevel all kept `*`).
`` is a project-wide qualifier, not a theory name; the Mathlib-shaped
...
'Mathoverflow1973.AdaptedWindows.flow_belt_passage' depends on axioms:
  [propext, Classical.choice, Quot.sound]
'Mathoverflow1973.ManifoldMorse.MorseSurgeryData' depends on axioms:
  [propext, Classical.choice, Quot.sound]
```
