...
| 3. `.{0}` pins left, forced by a declaration outside the packet | **80 (corrected; the receipt said 81)** |
...
**(corrected: four, not three.)**  Four declarations outside this packet pin the whole
`CategoryTheory/Sites/Leray` cluster to universe 0 and were not chased (the task forbids following a
pin out of the packet):
...
* **(added on correction)** `CategoryTheory.Sheaf.cohomologyAddCommGroup` —
  `Lib/Topology/Sheaves/Cohomology/AddCommGroup.lean:30` at the base,
  `instance cohomologyAddCommGroup {X : TopCat.{0}}`.  It is what pins
  `sheafCohomologyAddCommGroup {X : TopCat.{0}}` (`ResolutionTransgression.lean:67`), which wraps it.
  The pin is genuinely forced; the receipt as first written did not say by what.
...
lemmas, `match` and proof-term auxiliaries regenerated by the recompilation).  **(corrected)** the
one net-new constant (21719 → 21720, lost 249 / added 250) is
`CategoryTheory.Sheaf.Leray.higherDirectImageResolutionSheafificationIso._proof_5`, a declaration of
`HigherDirectImageSheafification.lean` (commit `cff67f3d`, this packet's).  It appears because the
universe-generalised `sheafification` / `homologyPresheaf` now leave a universe-unification
obligation to be discharged inside that def.  (The only other added-not-lost name in the merged
round-7 envdiff, `TopCat.SingularCochainSheaf.unit._proof_1`, belongs to packet 10.)  At head
`39f1d12b` the constant no longer exists: round 8 (`a140e4b1`) re-lifted the file.
...
Independent review: `Lib/reports/review-7-8/r7-packet-04.md` (ACCEPT WITH FINDINGS; every structural
claim — 18 files, one commit each, 335 docstrings, 26 pins lifted, 13 binders widened, no file
outside the packet, no `sorry`/`axiom`, the forced pins genuinely forced at the base — checks out
against the branch; no deletion, no rename).  These corrections are to this receipt's text only; no
Lean file was changed by them.

1. **"81 pins left" is 80 (finding 3), and the units are mixed.**  The per-file entries sum to
   18 + 20 + 6 + 8 + 12 + 2 + 1 + 1 + 1 + 9 + 1 + 1 = **80**, and the audit's per-file totals (106)
   minus the 26 lifted also give 80.  Corrected in place.  The units in that column are not uniform:
   `ResolutionTransgression.lean` "9 left" counts **declarations** (the file has 7 textual `.{0}`
   occurrences at the branch head, 12 before), while `StalkLocalCriterion.lean`'s "all 3" are
   **textual** pins on a single `variable` line (one declaration).  "26 generalised" is therefore
   12 + 8 + 3 (declarations) + 3 (textual).  None of this affects soundness.  Under the round-8 rule
   (`Lib/reviews/REVIEW-7-8.md` §4) the unit must be stated once and used throughout.

2. **The +1 constant is now named (finding 4).**  It is
   `CategoryTheory.Sheaf.Leray.higherDirectImageResolutionSheafificationIso._proof_5`; see the
   corrected paragraph in the environment-diff section.  Note that this cannot be re-checked from
   the tree: the packet's own `envdiff.json` was never committed (only
   `Lib/reports/round-7/envdiff-merged-d950428a.{json,txt}` exists), and at head the constant is gone
   because round 8 re-lifted the file.  Committing the per-packet JSON — the round-8 rule, now
   standing for all rounds — would have made this a one-line check.

3. **A fourth outside forcer was missing (finding 5).**  `sheafCohomologyAddCommGroup
   {X : TopCat.{0}}` (`ResolutionTransgression.lean:67`) wraps
   `CategoryTheory.Sheaf.cohomologyAddCommGroup`, which at the base is
   `instance cohomologyAddCommGroup {X : TopCat.{0}}` in
   `Lib/Topology/Sheaves/Cohomology/AddCommGroup.lean:30`.  Added to the forced-pin list above.

4. **Six `: Type` binders in `Lib/Geometry/Manifold/Morse/AdaptedWindows.lean` were found and left
   without being recorded (finding 2).**  This receipt says only "no universe pins or `: Type`
   binders were counted for this file" — true of the audit table, but item 4 of the packet is exactly
   binder widening, and for `Relative.lean` the receipt did say "none found".  The six are
   `exists_relative_level_surgery_system {E M : Type}`,
   `reaches_lower_of_excluded_critical_limit`, `reaches_old_lower_of_belt_avoidance`,
   `not_backward_basin_on_upper_level`, `transported_backward_basin_image {E M X : Type}` and
   `reaches_lower_in_regular_band` (lines 389, 456, 483, 504, 521, 564 at head `39f1d12b`, all still
   bare `Type`).  Neither round 8 nor anything later touched them.  They are recorded here as
   **found and left, reason untested** — whether they widen would need an elaboration; they are
   pinned neither by this receipt's four reasons nor obviously by their dependencies.  The item is
   on the fix list (`Lib/reviews/REVIEW-7-8.md` §3, packet 04: widen or record).

5. **Citation and docstring findings are code fixes, not receipt fixes.**  The reviewer's finding 1
   (`MorseCancellation.exists_clean_two_sheet_arc` is documented as "two **disjoint** embedded
   immersed surfaces"; the theorem has no disjointness hypothesis, only `hx : f x ∉ Set.range g`,
   `hy : g y ∉ Set.range f`), finding 6 (Hirsch "Ch. 8" — Isotopy — cited for the relative
   immersion/embedding theorems, which are Ch. 2 §2 and Ch. 3 §2; Godement II.1.1 cited for a
   filtered-colimit stalk criterion it does not state; Verdier II.4.3 / Kashiwara–Schapira §12–13 /
   Neeman §1.1 unconfirmed) and finding 7 (`sheafification_preservesFiniteColimits` cites Godement
   II.1.2 / Hartshorne II.1.2 for exactness, where both construct the associated sheaf) are listed in
   `Lib/reviews/REVIEW-7-8.md` §3 for the packet-04 fix agent.
