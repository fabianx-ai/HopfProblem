...
1. **ST 7325–8708 majority (85 decls)** — `ManifoldMorse.MorseSurgeryData`/`SurgeryWindows`
   ...
   to `MorseSurgeryData` (DT 10391, outside all lanes' ranges) and `SublevelDisk`/
...
2. **Recognition 11149–11254 Reeb block (4 decls)** — `nonempty_homeomorphSphere_of_two_critical_points`
   needs `Hemisphere.Sphere` (lane G). Probe still exact (below).
3. **Recognition leftovers** in the D1 ranges that reference the above.

## D1 axiom receipts

- `ManifoldMorse.exists_morse_function`: `[propext, Classical.choice, Quot.sound]`
...
- `ManifoldMorse.nonempty_homeomorphSphere_of_two_critical_points` (probed via scratch
  `import Hopf.Recognition`): `[propext, Classical.choice, Quot.sound]`

## D1 rename decision (deviation recorded)

No mandated rename list for D1. The `*` root is shared with lane F/G material that
...
rename (`ManifoldMorse.* → Morse.*` etc.) belongs to the post-F/G pass with the de-shim.
