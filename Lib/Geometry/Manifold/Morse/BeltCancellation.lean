...
Belt intersection points and their signs for the surgery data of a Morse critical point, the
Whitney cancellation of two belt intersections of opposite sign, the cell presentation of a
handle core, and the adapted-window transport used to bring a belt circle down to a lower
regular level.

The round-7 audit graded this module D: the two cancellation theorems
`ManifoldMorse.MorseSurgeryData.exists_belt_whitney_cancellation_of_opposite_signs` and
`exists_signed_belt_cancellation_step` carry the project's
`Module.finrank ℝ E = 6` and `finrank NegativeCoordinates = 2`, and most of the
`AdaptedWindows.*` rows are the project's instance of Milnor's second cancellation theorem
rather than the theorem.  What could be returned to `Hopf/Proof/` without breaking `Lib` has
been: the lower-transport tail and the radial parameter chart now live in
`Hopf.Proof.Geometry.Manifold.Morse.BeltCancellation`, and the general lemmas that were buried
here moved to the modules where they belong
(`RegularLevel.contMDiffWithinAt_iff_inclusion`, `contMDiffOn_iff_inclusion` to
`Lib.Geometry.Manifold.RegularLevel`; `PuncturedRadial.toSphere_fromSphere`, `deformation`,
`sphereHomotopyEquiv`, `LocalDegree.linearSphereEquiv`, `LocalDegree.BoundaryData.normalizedMap`
to `Lib.AlgebraicTopology.SingularHomology.LinearSphereAction`;
`ContinuousLinearMap.surjective_coprod_comp_left` to
`Lib.Geometry.Manifold.Transversality.Basic`).  The remainder stays only because
`Lib.Geometry.Manifold.Morse.SurgeryCollapse` -- a `Lib` module, which may not import `Hopf` --
consumes it; it should follow this file's tail into `Hopf/Proof/` when `SurgeryCollapse` is
itself split.

## Main declarations

* `ManifoldMorse.MorseSurgeryData.exists_belt_whitney_cancellation_of_opposite_signs`,
  `exists_signed_belt_cancellation_step` : cancellation of two belt intersections of opposite
  sign, in the project's dimension `6`, index `2` instance.
* `ManifoldMorse.MorseSurgeryData.HandleDomain`, `handleMap`, `coreMap`,
  `coreUnionHomotopyEquiv`, `coreCellPresentation` : the core of a handle presents the upper
  level as the lower level with a cell attached.
* `AdaptedWindows.exists_transverse_belt_circle_reaching_level_with_endpoints`,
  `exists_native_level_basin_transport` : transport of a belt circle to a lower regular level.

## References

* [John Milnor, *Lectures on the h-cobordism theorem*][milnor65], Thm 5.4 (first cancellation)
  and Thm 6.4 (second cancellation, via the Whitney trick).
* [John Milnor, *Morse theory*][milnor63], Thm 3.2 (the core of a handle as a cell).
...
Morse theory, Whitney trick, handle cancellation, belt sphere
-/
