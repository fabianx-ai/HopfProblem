...
def stalkSheafificationUnitNatTrans (y : Y) :
  ...
  naturality {P Q} η := by
    change (TopCat.Presheaf.stalkFunctor AddCommGrpCat y).map η ≫
    ...
    ((TopCat.Sheaf.forget AddCommGrpCat Y).mapHomologicalComplex
      ...
      (NatTrans.mapHomologicalComplex
        (CategoryTheory.toSheafification
          (Opens.grothendieckTopology Y) AddCommGrpCat)
        (ComplexShape.up ℕ)).app (underlyingPresheafComplex K) := by
  apply HomologicalComplex.Hom.ext
  funext n
  change (TopCat.Sheaf.forget AddCommGrpCat Y).map
...
theorem stalkHomologyPresheafIso_hom_comp_sheafificationUnit
  ...
  have hcomplex :
    ...
    apply HomologicalComplex.Hom.ext
    funext i
    change (TopCat.Presheaf.stalkFunctor AddCommGrpCat y).map
...
def higherDirectImageResolutionSheafificationStalkIso
  ...
  ((TopCat.Sheaf.forget AddCommGrpCat Y ⋙
    ...
    (asIso ((TopCat.Presheaf.stalkFunctor AddCommGrpCat y).map
...
theorem higherDirectImageResolutionSheafificationStalkIso_eq
  ...
  rw [(TopCat.Sheaf.forget AddCommGrpCat Y).map_comp,
    (TopCat.Presheaf.stalkFunctor AddCommGrpCat y).map_comp]
  change
    (TopCat.Sheaf.forget AddCommGrpCat Y ⋙
        TopCat.Presheaf.stalkFunctor AddCommGrpCat y).map
      ...
      inv ((TopCat.Presheaf.stalkFunctor AddCommGrpCat y).map
        (TopCat.SheafificationLocal.unit
