...
/-!
# Resolution coordinates on the target of the Postnikov page-two differential

This file compares the actual Leray-page coordinate at bidegree `(2,q)` with the generic
Postnikov target `Ext²` coordinate.  The comparison keeps the non-definitional total-degree
transport visible, moves all homology comparison maps through the single-object shift adapters,
and introduces no scalar or sign.
-/
