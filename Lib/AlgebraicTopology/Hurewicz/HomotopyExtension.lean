...
`SecondHurewicz.SimplyConnected.extendBoundaryHomotopy f h h0 : C(unitInterval ×
...
* `SecondHurewicz.SimplyConnected.cylinderRetraction`: retraction onto
  `bottomOrSide n`.
* `SecondHurewicz.SimplyConnected.extendBoundaryHomotopy`: the extension, with
