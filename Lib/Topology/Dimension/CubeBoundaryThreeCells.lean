...
The mesh-`h` subdivision of `∂[-1,1]³` has three kinds of cells: the lattice vertices, the
mesh edges joining a vertex to its successor in one coordinate direction, and the mesh squares
spanned by two coordinate directions.  This file develops their geometry: each cell has a unique
coordinatewise lower corner, a cell determines the directions and parameters presenting it, the
relative interiors of distinct squares are disjoint, every boundary point lies in some square,
and each of the three families is finite.
...
/-! ## Enclosure of a boundary point by a cell

Every point of the cube boundary lies in a mesh square, and a point of a mesh edge lies within
half a mesh of one of its endpoints.  The declarations below establish the coordinate and
interval estimates behind these two enclosure statements. -/
